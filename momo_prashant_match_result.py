...
def calculate_metrics_and_rank(df_all_stocks):
    ...
    metrics_df = (
        ...
        .agg([
            ...
            (pl.col("close") * pl.col("volume")).tail(TRADING_DAYS_PER_YEAR).mean().alias("average_volume"),
            (pl.col("close") * pl.col("volume")).tail(TRADING_DAYS_PER_YEAR).median().alias("median_volume"),
    ...
    top_700_df = filtered_df.sort("average_volume", descending=True).head(700)
